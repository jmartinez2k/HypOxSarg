...
```{r}
#Current Data
setwd("C:/Users/jamar/R/HypOxSarg/code/RawData")
...
# After Hauri et al., 2013 (https://doi.org/10.1002/grl.5028): 
  ...
  ## Add ID columns for 4 thresholds (<5, <4, <3, and <2 µmol/kg) 
...
### WORKING WITH INDIVIDUAL OBSERVATIONS  ---
  ...
  event_hyp_2$Date <- as.POSIXct(event_hyp_2$Date, tz = "UTC")
  event_hyp_3$Date <- as.POSIXct(event_hyp_3$Date, tz = "UTC")
  event_hyp_4$Date <- as.POSIXct(event_hyp_4$Date, tz = "UTC")
  event_hyp_5$Date <- as.POSIXct(event_hyp_5$Date, tz = "UTC")
...
#convert miniDOT data to time series objects
X1 <- X1[!is.na(as.POSIXlt(X1$`UTC_Date_&_Time`, format="%Y-%m-%d %H:%M:%S")), ]
X1_ts <- xts(as.numeric(X1$`Dissolved Oxygen`), order.by = as.POSIXlt(X1$`UTC_Date_&_Time`, format="%Y-%m-%d %H:%M:%S"))
X2 <- X2[!is.na(as.POSIXlt(X2$`UTC_Date_&_Time`, format="%Y-%m-%d %H:%M:%S")), ]
...
sarg_sat <- read_excel("Sargassum_NDVI_DarkWater_Satellite_Data.xlsx", 
    col_types = c("date", "numeric", "text", 
        "numeric", "numeric")) %>% 
  rename(mask_area_m2=`Mask Area_m2`) %>% 
  rename(per_area=`Area of Coverage_percent`)
...
# Add a year column
sarg_sat_AOI1_sarg <- sarg_sat_AOI1_sarg %>% mutate(Year = year(Date))
sarg_sat_AOI2_sarg <- sarg_sat_AOI2_sarg %>% mutate(Year = year(Date))
sarg_sat_AOI3_sarg <- sarg_sat_AOI3_sarg %>% mutate(Year = year(Date))

# Function to calculate mean and SD by year
calculate_stats <- function(data, AOI) {
  data %>%
    group_by(Year) %>%
    summarise(
      Mean_Percent_Cover = mean(per_area, na.rm = TRUE),
      SD_Percent_Cover = sd(per_area, na.rm = TRUE)
    ) %>%
    mutate(AOI = AOI) %>%
    select(AOI, Year, Mean_Percent_Cover, SD_Percent_Cover)
}

# Apply function to each AOI
stats_AOI1 <- calculate_stats(sarg_sat_AOI1_sarg, "1")
stats_AOI2 <- calculate_stats(sarg_sat_AOI2_sarg, "2")
stats_AOI3 <- calculate_stats(sarg_sat_AOI3_sarg, "3")

# Combine results
summary_stats <- bind_rows(stats_AOI1, stats_AOI2, stats_AOI3)

print(summary_stats)

event_hyp_5$Date <- as.POSIXct(event_hyp_5$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
event_hyp_4$Date <- as.POSIXct(event_hyp_4$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
event_hyp_3$Date <- as.POSIXct(event_hyp_3$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
event_hyp_2$Date <- as.POSIXct(event_hyp_2$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")

event_hyp_5_24$Date <- as.POSIXct(event_hyp_5_24$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
event_hyp_4_24$Date <- as.POSIXct(event_hyp_4_24$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
event_hyp_3_24$Date <- as.POSIXct(event_hyp_3_24$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
event_hyp_2_24$Date <- as.POSIXct(event_hyp_2_24$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")

sarg_sat_AOI1_sarg$Date <- as.POSIXct(sarg_sat_AOI1_sarg$Date, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
...
DO_plot_23 =
  ...
  # Ensure fill is recognized as a factor
  scale_fill_manual(values = c("5" = "#969696", "4" = "#606060", "3" = "#505050","2" = "#e31a1c"),
                    labels = c("2", "3", "4", "5"),
                    name = expression("Thresholds (mg" ~ L^-1 * ")")) +
  
  # Mapping fill inside aes()
  geom_rect(aes(xmin = as.POSIXct("2023-08-18 UTC"), 
                xmax = as.POSIXct("2023-09-26 UTC"), 
                ymin = 0, ymax = 2, fill = factor("2")), 
            alpha = 1.0, color = NA, show.legend = TRUE) +
  
  geom_rect(aes(xmin = as.POSIXct("2023-08-18 UTC"), 
                xmax = as.POSIXct("2023-09-26 UTC"), 
                ymin = 2, ymax = 3, fill = factor("3")), 
            alpha = 0.9, color = NA, show.legend = TRUE) +
  
  geom_rect(aes(xmin = as.POSIXct("2023-08-18 UTC"), 
                xmax = as.POSIXct("2023-09-26 UTC"), 
                ymin = 3, ymax = 4, fill = factor("4")), 
            alpha = 0.9, color = NA, show.legend = TRUE) +
  
  geom_rect(aes(xmin = as.POSIXct("2023-08-18 UTC"), 
                xmax = as.POSIXct("2023-09-26 UTC"), 
                ymin = 4, ymax = 5, fill = factor("5")), 
            alpha = 0.9, color = NA, show.legend = TRUE) +
  ...
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1.0),
        axis.title.x = element_text(size = 35, margin = margin(b = 10)),
        axis.text.x = element_text(size = 35, margin = margin(t = 5)),
        axis.title.y = element_text(size = 35, margin = margin(r = 10)),
        axis.text.y = element_text(size = 35, margin = margin(l = 5, t = 5)),
...
temp_plot_23 =
  ...
  theme(panel.border = element_rect(color = "black", fill = NA, size = 1.0),  # Create a border around the plot
        axis.title.x = element_text(size = 35, margin = margin(b = 10)),  # Add space below x-axis title
       #axis.text.x = element_text(size = 35, margin = margin(t = 5)),     # Add space above x-axis numbers
        axis.title.y = element_text(size = 35, margin = margin(r = 10)),   # Add space to the right of y-axis title
        axis.text.y = element_text(size = 35, margin = margin(l = 5)),   # Add space to the left of y-axis numbers
...
stick_current_plot_23 = 
    ...
    scale_color_manual(values = my_colors) +
    ylab(expression("Current (cm" ~ s^-1 * ")"))+
    xlab("")+
    scale_color_manual(values = my_colors) +
    labs(subtitle = "(C)") +
    theme(panel.border = element_rect(color = "black", fill = NA, size = 1.0),  # Create a border around the plot
        axis.title.x = element_text(size = 35, margin = margin(b = 10)),  # Add space below x-axis title
    #    axis.text.x = element_text(size = 35, margin = margin(t = 5)),     # Add space above x-axis numbers
        axis.title.y = element_text(size = 35, margin = margin(l = -10,r = 0)),   # Add space to the right of y-axis title
        axis.text.y = element_text(size = 35, margin = margin(l = 5)),   # Add space to the left of y-axis numbers
        axis.text.x = element_blank(),
        axis.ticks.length = unit(0.5, "cm"),
        axis.ticks = element_line(size = 1),plot.margin = margin(t = 15, l = 15),# Adjust axis tick size
...
ggsave("DO_plot_23.jpg",DO_plot_23,width = 32, height = 24)
```
...
DO_plot_24 =
  ...
  scale_fill_manual(values = c("2" = "#e31a1c", "3" = "#505050", "4" = "#606060", "5" = "#969696"),
...
# Combine the 2023 and 2024 plots side by side
ts_plot <- cowplot::plot_grid(ts_plot_23, ts_plot_24, ncol = 2, align = "h")

# Extract the legend from one of the plots (e.g., DO_plot_23)
legend <- cowplot::get_legend(DO_plot_23 + theme(legend.position = "bottom"))

print(legend)

# Combine the main plot and the legend
fig2 <- cowplot::plot_grid(ts_plot, legend, ncol = 1, rel_heights = c(1, 0.1))

# Save the final plot
ggsave("fig2.pdf", fig2, width = 32, height = 24)
ggsave("fig2.jpg", fig2, width = 32, height = 24)
...
plot_duration_threshold_23 <- ggplot() +
  ...
  geom_bar(data = event_hyp_5, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  geom_bar(data = event_hyp_4, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  geom_bar(data = event_hyp_3, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  geom_bar(data = event_hyp_2, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  scale_fill_manual(values = c("5" = "#969696", "4" = "#606060", "3" = "#505050", "2" = "#e31a1c")) +
  scale_x_datetime(breaks = common_breaks_23, expand = common_expand, labels = function(x) format(x, "%m-%d")) +
...
plot_duration_threshold_24 <- ggplot() +
  geom_hline(yintercept = seq(0, 20, 2.5), color = "gray", linetype = "solid", size = 1, alpha = 0.5) +
  geom_bar(data = event_hyp_5_24, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  geom_bar(data = event_hyp_4_24, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  geom_bar(data = event_hyp_3_24, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  geom_bar(data = event_hyp_2_24, aes(x = Date, y = dur_hrs, fill = threshold), 
           stat = "identity", position = "stack", alpha = 0.9) +
  scale_fill_manual(values = c("#969696", "#606060", "#505050", "#e31a1c")) +
...
ggsave("fig3.jpg",fig3, width = 32, height = 24)
...
min_DO_Model_mgL <- ggplot(min_O2_per_day, aes(x = sarg_kg, y = min_O2_mg_L, color = factor(temp), shape = factor(tau), linetype = factor(tau))) +
  ...
  #scale_color_manual(name = "Temperature:", values = c("#2166ac", "#fdae61"), labels = c("28°C", "31°C")) +
...
fig5 <- cowplot::plot_grid(sargasso_plot, NULL, legend, ncol = 1, rel_heights = c(1, 0.0001, 0.05), align = "v")


ggsave("fig5.jpg",fig5,width = 21, height = 24)
#ggsave("fig5.pdf",fig5,width = 21, height = 24)
...
figS1 = raw_DO_median_2023 + raw_DO_median_2024 +  plot_layout(ncol = 1)

#ggsave("raw_DO_median_2024.jpg",raw_DO_median_2024,width = 32, height = 24)

ggsave("figS1.jpg",figS1,width = 32, height = 24)
```

## Satellite Sargassum Figures

```{r}
# Isla Magueyes Sargassum Satellite Data Visualization
...
# La Parguera Sargassum Satellite Data Visualization
...
figS3 = cowplot::plot_grid(sat_plot_sarg_sum, sat_plot_sarg_win, ncol = 2)

ggsave("figS3.jpg",figS3,width = 32, height = 24)
